...
- [x] Also detect launchd plists / systemd units running cloudflared and offer to take them over. *(Running ones are detected as foreign connectors marked "background service" and can be stopped; their routes can be imported. Stopped, installed-only services aren't listed: nothing is served through them.)*
