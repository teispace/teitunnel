...
- 2026-09-23: Website polish (D-081): landing page motion (scroll reveal, hero, plan and CLI demos, marquee; static with Reduce Motion or no JS), header blended into the hero with section links, Teispace footer; SEO (canonical URLs, per-page social cards, sitemap, robots, llms.txt, JSON-LD); docs expanded: Use cases (7 guides), What is Cloudflare Tunnel, comparison, Troubleshooting, install with direct downloads and uninstall, FAQ rewritten, run modes for every platform; screenshots re-shot as WebP (3.3 MB → 1.3 MB for twice as many).
