...
## Next up
0. **Release work now (D-074, maintainer 2026-09-23):** ~~M6-02 updater~~ (D-075) → ~~M6-03 release workflow + release-please~~ → ~~M6-07 download pages~~ (D-076) → maintainer setup (Apple cert + API key, updater key, `release` environment, Pages) → dry run → v0.1.0 public beta (website only; Windows unsigned until SignPath) → M6-04 channels.
