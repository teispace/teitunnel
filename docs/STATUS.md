...
## Next up
0. **M10/M11 done** ([plan](plans/M10-parity.md), Cloudify analysis in [research/cloudify.md](research/cloudify.md)). All follow-ups done. Next: release work (signing with the maintainer's Developer ID, updater, release automation, channels incl. ghcr.io image and winget/Homebrew, website deploy to teitunnel.teispace.com: GitHub Pages + Cloudflare DNS CNAME `teitunnel` → `teispace.github.io`, then point the app's Help link there), done last per the maintainer (2026-09-23).
