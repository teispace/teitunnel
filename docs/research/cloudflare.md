...
- Verified 2026-09-23 (cloudflare-docs `fundamentals/api/how-to/account-owned-token-template.mdx` and PR #33557):
  ...
  - Re-checked 2026-09-23 (same doc, "Access permissions" table): `access` = Access applications (dashboard: Access: Apps and Policies), `access_acct` = Access organizations (Access: Organizations, Identity Providers, and Groups). Editing an existing token's permissions in the dashboard keeps its value (only **Roll** changes it).
...
## Docs claims checked (2026-09-26)

- **524 timeout is 125 s, not 100 s.** Cloudflare's [connection limits](https://developers.cloudflare.com/fundamentals/reference/connection-limits/) list "Proxy Read Timeout | 125 | 524 | Yes, for Enterprise zones", and the [error 524 page](https://developers.cloudflare.com/support/troubleshooting/http-status-codes/cloudflare-5xx-errors/error-524/) says "the default 125 seconds"; Enterprise can raise it to 6,000 s. Every "100 seconds" in the docs, the app's help text, and Lens and MCP comments now says 125. The SSE keep-alive (25 s) and the breakpoint limit (60 s) were already well inside it. Proxy Idle Timeout is 900 s (520).
- **Zero Trust Free: 50 users.** [Access pricing](https://www.cloudflare.com/sase/products/access/): Free "$0 forever", "50 user limit"; Pay-as-you-go $7 per user a month, no user limit.
- **Windows `cmdkey` and Linux `secret-tool` recipes** (project-file guide): checked against the source of the keyring stores the app uses (keyring 4.2 `v1` defaults). Windows: `windows-native-keyring-store` 1.1 names a generic credential `<user>.<service>` (empty prefix and suffix, `.` delimiter) and stores the password as UTF-16, as `cmdkey /generic:… /user:… /pass` does ([cmdkey](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/cmdkey): "/pass … If `<password>` isn't supplied, it will be requested"). Linux: `zbus-secret-service-keyring-store` 1.0 searches by exactly `service` and `username` (no `target` unless set), which `secret-tool store … service … username …` writes. Not yet run on a real Windows or Linux desktop.
- **Still unverified:** whether signing in again with OAuth offers scopes declined earlier (permissions page). Cloudflare doesn't document its consent screen's behaviour; needs a live sign-in.
