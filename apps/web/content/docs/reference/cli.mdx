---
title: Command line
description: "The teitunnel-cli reference: add routes, share ports and manage Cloudflare Tunnel from the terminal, with every command, option and origin setting."
---
...
The app includes it. Turn it on in **Settings ▸ General ▸ Command line ▸ Install**:

- **macOS:** a link in Homebrew's `bin` (or `/usr/local/bin`). If Teitunnel can't write
  there, Settings shows the one `sudo ln -s …` command to run instead.
- **Windows:** a copy in your user's `WindowsApps` folder, which is already on your PATH.
- **Linux:** the `.deb` and `.rpm` put it in `/usr/bin`; with the AppImage, it goes to
  `~/.local/bin`.

It updates with the app. For servers without the app, download it on its own from the
[download page](/download/#cli).

## Commands

```sh
teitunnel-cli routes                         # this machine's routes and their status
...
teitunnel-cli networks                       # the ranges this machine shares
...
```sh
teitunnel-cli route add secure.example.com https://localhost:8443 --no-tls-verify
teitunnel-cli route add app.example.com 5173 --host-header localhost
teitunnel-cli route add slow.example.com 8000 --connect-timeout 60 --keep-alive-timeout 300
```
