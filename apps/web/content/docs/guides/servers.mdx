...
The app is for your computer. On a server, `teitunnel` does the same job from the
...
does have a keychain, `teitunnel setup` stores it there once instead.

## Any Linux server (VPS, EC2, Azure VM, Compute Engine)

```sh
teitunnel route add app.example.com 3000 --yes   # creates this server's tunnel
...
Without systemd, run `teitunnel up` under the supervisor you have (runit, s6,
...
The container's health check is `teitunnel routes --check`, which fails unless every
route is live.
...
`kubectl exec deploy/teitunnel -- teitunnel route add app.example.com http://web.default.svc:80 --yes`.

## Web dashboard and API
`teitunnel serve` runs the tunnels like `up` and adds a small dashboard: every
...
itself with a login (`teitunnel route add admin.example.com http://127.0.0.1:8765
--allow you@example.com`), or pass `--listen 0.0.0.0:8765 --allow-remote` behind a TLS
proxy with `--secure-cookies`.

For automation, create an API key (`teitunnel api-key create deploy`, shown once) and
