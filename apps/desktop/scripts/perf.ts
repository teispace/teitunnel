...
// a log at 2,000 lines/s and a 3,600-point chart at 20 Hz, rendered together.
// Reports frame-interval percentiles, long frames and DOM size over time.
//
// Usage: node scripts/perf.ts [seconds=30]
...
try {
  ...
  await page.goto(new URL("dev/stress", base).toString());
  ...
  const result = {
    ...
    logLinesPerSecond: Math.round(stats.logLines / (seconds + 2)),
